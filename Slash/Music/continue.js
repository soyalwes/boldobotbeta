...
module.exports = {
  data: new SlashCommandBuilder().setName("continue").setDescription("Continua reprodiciendo la musica"),
  ...
  async run(client, interaction) {
    ...
    if(!queue)return interaction.reply({content:"No hay canciones reprodiciendose"})

    if(!queue.pause){
        return interaction.repl({content:"La cancion no esta pausada"})
    }
    try{
        client.distube.resume(interaction.member.voice.channel)
        interaction.reply({content:"▶|Continuando cancion"})
        return
    } catch (e) {
        console.log(e)
        const errorEmbed = new MessageEmbed()
        .setTitle("❌|Error")
        .setDescription("No se a podido continuar la musica")
        .setColor("RED")
        .setTimestamp();
